    ...
    <div class="detail-page">
        <section class="detail-hero" v-if="scenicSpotDetail.coverUrl">
            ...
            <div class="hero-content">
                <div class="hero-breadcrumb">
                    <span @click="$router.push('/viewer')">首页</span>
                    <span class="sep">/</span>
                    <span>景点详情</span>
                </div>
                ...
                <div class="hero-actions" v-if="isUserLoggedIn">
        ...
        <section class="detail-content">
            <div class="content-inner">
                ...
                <div v-if="tabIndex === 2">
                    <Comment v-if="isUserLoggedIn && userInfo" :currentUserId="Number(userInfo.id)"
                        :userAvatar="String(userInfo.avatar)" :username="String(userInfo.username)"
                        :contentId="Number(scenicSpotDetail.id)" :contentType="contentModule" />
                ...
                <div v-if="tabIndex === 3" class="route-section">
                    <ViewerScenicSpotRouteItem :scenic-spot-id="scenicSpotDetail.id" />
                </div>
                ...
                <div v-if="tabIndex === 5" class="ticket-section">
                    <ViewerScenicSpotTickets :scenic-spot-id="scenicSpotDetail.id" />
                </div>
            </div>
        </section>
...
import ViewerScenicSpotRouteItem from "./ViewerScenicSpotRouteItem.vue"
import ViewerScenicSpotTickets from "./ViewerScenicSpotTickets.vue"
import { getUserInfo, getToken } from "@/utils/storage"
import Comment from "@/components/Comment.vue"
import { useRouter } from "vue-router"
import { onBeforeRouteLeave } from 'vue-router'
import { getScenicSpotDetailByViewerApi, queryStrategyByScenicSpotIdByViewerApi, queryCategoryListByScenicSpotIdByViewerApi } from "@/api/viewer-api"
...
const goScenicSpotStrategySearch = () => { window.open(`/viewer-scenic-spot-strategy-home`) }
const viewStrategy = (id: number) => { window.open(`/viewer-scenic-spot-strategy-detail?scenicSpotStrategyId=${id}`) }
...
const upvoteOperation = async () => {
    if (!isUserLoggedIn.value) { router.push('/login'); return }
    try {
...
const getScenicSpotDetail = async (id: number | null) => {
    ...
    try {
        const { data } = await getScenicSpotDetailByViewerApi(id)
...
const getScenicSpotStrategyList = async (id: number | null) => {
    if (!id) return
    try {
        const { data } = await queryStrategyByScenicSpotIdByViewerApi(id)
        scenicSpotStrategyListVO.value = data
    } catch (error: any) { console.error("查询景点攻略异常：", error) }
}

const getCategoryList = async (id: number | null) => {
    if (!id) return
    try {
        const { data } = await queryCategoryListByScenicSpotIdByViewerApi(id)
...
onMounted(async () => {
    ...
    if (scenicSpotId.value) {
        ...
        await loadUserInfo()
        await getCategoryList(scenicSpotId.value)
    }
})
</script>
...
.login-prompt { text-align: center; padding: 60px 20px;
    p { font-size: 16px; color: var(--text-light); margin-bottom: 20px; }
    .login-btn { display: inline-flex; padding: 10px 28px; background: var(--primary-color); color: white; border-radius: 100px; font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.3s;
