<template>
    <div class="scenic-route-detail-container">
        <!-- 景点路线项列表 -->
        <div>
            <UserRouteItems @routes="handleSelectRoutesChildren" :scenicSpotRouteItemList="scenicSpotRouteItemList" />
        </div>
    </div>
</template>
...
import {
    queryScenicSpotRouteItemByViewerApi
} from "@/api/viewer-api"
import {
    getScenicSpotDetailByViewerApi
} from "@/api/viewer-api"
...
const handleSelectRoutesChildren = (route: any) => {
    // 跳转路线节点页
    window.open(`/scenic-spot-route-nodes-viewer?scenicSpotRouteItemId=${route.id}`);
}
...
const queryScenicSpotRouteItem = async () => {
    try {
        ...
        const { data } = await queryScenicSpotRouteItemByViewerApi(scenicSpotRouteItemQueryDTO)
