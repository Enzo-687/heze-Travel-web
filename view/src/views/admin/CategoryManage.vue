<template>
    <div class="category-container">
        ...
        <div class="top-header">
            <div class="nav-left">
                <div>
                    <Input placeholder="搜索根节点，按Enter↩︎键" @listener="handleSearch" />
                </div>
                <el-button type="primary" @click="handleRootCategory">
                    新增根类别
                </el-button>
            </div>
            <div class="nav-right">
        ...
        <div class="table-container">
            <el-table ref="tableRef" :data="categoryList" row-key="id" lazy :load="loadChildren"
                ...
                <el-table-column label="操作" width="250" align="center" fixed="right">
                    <template #default="{ row }">
                        <el-button v-if="row.level < 4" type="info" size="small" @click="handleAddChild(row)"
                            :icon="Plus">
                            添加子类
                        </el-button>
                        ...
                        <el-button @click.stop type="info" size="small" @click="handleDelete(row)" :icon="Delete">
                            删除
                        </el-button>
                    </template>
                </el-table-column>
            ...
            <div v-if="isRootQuery" class="pagination-container">
                <el-pagination v-model:current-page="CategoryQueryDTO.current" v-model:page-size="CategoryQueryDTO.size"
...
import { queryCategory, addCategory, updateCategory, delCategory } from "@/api/category-api"
import type { CategoryQueryDto, Category } from "@/types/category"
...
// 是否正在查询根节点
const isRootQuery = computed(() => CategoryQueryDTO.parentId === -1)
...
const checkRealChildren = async (row: any): Promise<boolean> => {
    try {
        ...
        // 如果没有缓存，查询数据库确认
        const childQuery: CategoryQueryDto = {
            parentId: row.id
        }
        const response = await queryCategory(childQuery)
...
const updateNodeHasChildren = (nodeId: number, hasChildren: boolean) => {
    ...
    findAndUpdate(categoryList.value)
}

// 加载根节点分类（分页查询）
const loadRootCategories = async () => {
    try {
        CategoryQueryDTO.parentId = -1

        const response = await queryCategory(CategoryQueryDTO)
        ...
        console.error("查询根分类异常：", error)
        ElMessage.error("加载分类数据失败")
    }
}

// 懒加载子节点
const loadChildren = async (row: Category, treeNode: any, resolve: (data: Category[]) => void) => {
    try {
        if (row.hasChildren) {
            const childQuery: CategoryQueryDto = {
                name: '',
...
const handleSearch = (name: any) => {
    CategoryQueryDTO.current = 1
    CategoryQueryDTO.name = name
    if (searchForm.level) {
        CategoryQueryDTO.level = searchForm.level
    } else {
        CategoryQueryDTO.level = undefined
    }
...
const handleRootCategory = async () => {
    try {
        const { value } = await ElMessageBox.prompt('请输入图书类别根节点名称', '添加根类别', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            inputPattern: /.+/,
...
// 添加子分类
const handleAddChild = async (row: Category) => {
    try {
        console.log("当前节点：", row);

        const { value } = await ElMessageBox.prompt(`请输入 ${row.name} 的子分类名称`, '添加子分类', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            inputPattern: /.+/,
            inputErrorMessage: '分类名称不能为空'
        ...
        category.name = value
        category.parentId = row.id as number
        try {
...
const handleEdit = async (row: Category) => {
    try {
        ...
        try {
            await updateCategory(categoryUpdateDTO as any)
            row.name = value
...
const handleCurrentChange = (current: number) => {
    CategoryQueryDTO.current = current
    loadRootCategories()
}
