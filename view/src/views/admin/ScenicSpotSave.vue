    ...
    <div class="scenic-spot-save-container">
        ...
        <div class="page-header">
            <el-page-header icon="ArrowLeft" content="景点新增" @back="router.go(-1)" class="header-content">
                <template #content>
                    <h2 class="header-title">景点信息新增</h2>
                </template>
            </el-page-header>
        </div>
        ...
        <div class="main-container">
            ...
            <div class="action-bar">
                ...
                <div class="action-right">
                    ...
                    <el-button round type="primary" size="mini" class="action-btn submit-btn" @click="saveScenicSpot"
            ...
            <div class="content-card">
                ...
                <div class="form-section" v-if="activeStepIndex === 1">
                    <div class="form-group">
                        ...
                        <el-input v-model="scenicSpotSaveDTO.title" placeholder="请输入景点标题（100字以内）" maxlength="100"
                    ...
                    <div class="form-group">
                        ...
                        <el-input type="textarea" :rows="4" placeholder="请输入景点摘要（200字以内）"
                            v-model="scenicSpotSaveDTO.summary" maxlength="200" show-word-limit size="large"
                ...
                <div class="form-section" v-if="activeStepIndex === 2">
                    <div class="region-group">
                        ...
                        <div class="region-level" v-if="childrenRegionList.length">
                            ...
                            <el-radio-group v-model="scenicSpotSaveDTO.regionId" class="radio-group">
                ...
                <div class="form-section" v-if="activeStepIndex === 4">
                    ...
                    <WangEditor :receive-content="contentWithImage" height="500px" :api="uploadApi"
                        @on-listener="handleContentWithImageChange" class="editor-wrapper" />
                </div>
...
import type { ScenicSpotQueryDTO, ScenicSpotSaveDTO } from "@/types/scenic-spot"
...
import { saveScenicSpotApi } from "@/api/scenic-spot-api"
...
const scenicSpotSaveDTO = reactive<Partial<ScenicSpotSaveDTO>>({
    categoryIds: []
})
const cover = ref('')
...
// 景点新增
const saveScenicSpot = async () => {
    // 参数校验，封装数据，发请求，根据请求结果做后续动作
    if (!scenicSpotSaveDTO.title) {
    ...
    // 封面
    scenicSpotSaveDTO.cover = cover.value
    // 内容
    scenicSpotSaveDTO.content = contentWithImage.value
    // 图册封装成指定的数据结构
    ...
    scenicSpotSaveDTO.scenicSpotBrochureList = scenicSpotBrochureList

    // 设置类别关联数据
    // childrenPlusCategoryId - childrenCategoryId - parentCategoryId
    if(!scenicSpotSaveDTO.categoryIds){
        scenicSpotSaveDTO.categoryIds = []
    }
    if (childrenPlusCategoryId.value) {
        scenicSpotSaveDTO.categoryIds.push(childrenPlusCategoryId.value)
    }
    if (childrenCategoryId.value) {
        scenicSpotSaveDTO.categoryIds.push(childrenCategoryId.value)
    }
    if (parentCategoryId.value) {
        scenicSpotSaveDTO.categoryIds.push(parentCategoryId.value)
    }
    try {
        await saveScenicSpotApi(scenicSpotSaveDTO as any)
        ElMessage.success('景点新增成功')
        router.go(-1) // 返回上一页
    } catch (error: any) {
        ElMessage.error('景点新增异常：', error.message)
    }
