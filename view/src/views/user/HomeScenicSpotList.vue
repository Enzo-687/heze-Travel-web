  ...
  <div class="spot-page">
    ...
    <section class="filter-section">
      <div class="filter-inner">
        ...
        <div class="filter-actions" v-if="scenicSpotQueryDTO.title || activeTheme || selectedCountyId">
      ...
      <div class="spot-inner">
        ...
        <div v-if="scenicSpotListVOS.length" class="pagination">
          <el-pagination v-model:current-page="scenicSpotQueryDTO.current"
            v-model:page-size="scenicSpotQueryDTO.size" :page-sizes="[8, 12, 24]" :total="totalPageCount"
...
const selectCounty = async (id: number | null) => {
  ...
  if (id) {
    try {
      const { data } = await queryListApi({ parentId: id })
      townList.value = data
...
const goDetail = (id: number) => { window.open(`/scenic-spot-info?scenicSpotId=${id}`) }
...
const handleSizeChange = async (size: number) => { scenicSpotQueryDTO.size = size; scenicSpotQueryDTO.current = 1; await fetchScenicSpot() }
const handleCurrentChange = async (current: number) => { scenicSpotQueryDTO.current = current; await fetchScenicSpot(); window.scrollTo({ top: 0, behavior: 'smooth' }) }
...
const fetchScenicSpot = async () => {
  ...
  } catch (error: any) { ElMessage.error('景点查询异常') }
}

onMounted(async () => {
  try {
    const { data } = await queryListApi({ parentId: -1 })
    countyList.value = data
  } catch (error: any) { ElMessage.error('查询地区信息异常') }
  await fetchScenicSpot()
})
</script>

<style lang="scss" scoped>
.spot-page { min-height: 100vh; background: #FAFAFA; }

.hero-section {
  position: relative; height: 360px; overflow: hidden;
  .hero-bg { position: absolute; inset: 0;
    .hero-gradient { position: absolute; inset: 0; background: linear-gradient(135deg, #0A0A0A 0%, #1A0A0E 30%, #2A1018 60%, #0A0A0A 100%); }
    .hero-particles { position: absolute; inset: 0;
      .particle { position: absolute; border-radius: 50%; background: rgba(166, 27, 46, 0.2); animation: floatUp linear infinite;
        @keyframes floatUp { 0%, 100% { transform: translateY(0) scale(1); opacity: 0; } 5% { opacity: 0.6; } 95% { opacity: 0.6; } 50% { transform: translateY(-80px) scale(1.3); opacity: 0.3; } }
      }
    }
  }
  .hero-content { position: relative; z-index: 2; max-width: 1200px; margin: 0 auto; padding: 100px 40px 0;
    .hero-badge { display: inline-block; padding: 6px 16px; background: rgba(166, 27, 46, 0.2); border: 1px solid rgba(166, 27, 46, 0.4); border-radius: 100px; color: #E38995; font-size: 12px; font-weight: 600; letter-spacing: 4px; margin-bottom: 20px; }
    .hero-title { font-family: var(--font-serif); font-size: clamp(32px, 5vw, 48px); font-weight: 700; color: white; letter-spacing: 6px; margin-bottom: 12px; }
    .hero-desc { font-size: 16px; color: rgba(255, 255, 255, 0.4); letter-spacing: 4px; font-family: var(--font-serif); margin-bottom: 24px; }
...
.filter-section {
  padding: 32px 0; background: white; border-bottom: 1px solid rgba(0, 0, 0, 0.04);
  .filter-inner { max-width: 1200px; margin: 0 auto; padding: 0 40px; }
  ...
  .filter-actions { margin-top: 8px;
    .reset-btn { display: inline-flex; align-items: center; gap: 6px; padding: 6px 16px; border-radius: 100px; font-size: 13px; font-weight: 500; color: var(--text-secondary); background: #F5F5F5; cursor: pointer; transition: all 0.3s;
      &:hover { background: rgba(166, 27, 46, 0.06); color: var(--primary-color); }
    }
  }
}

.spot-section { padding: 40px 0 80px;
  .spot-inner { max-width: 1200px; margin: 0 auto; padding: 0 40px; }
}
...
.empty-state { text-align: center; padding: 80px 20px;
  ...
  .empty-action { display: inline-flex; padding: 10px 28px; background: var(--primary-color); color: white; border-radius: 100px; font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.3s; &:hover { background: var(--primary-light); transform: translateY(-1px); } }
}

.spot-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 24px; }

.spot-card {
  background: white; border: 1px solid rgba(0, 0, 0, 0.06); border-radius: 16px;
  overflow: hidden; cursor: pointer; transition: all 0.35s cubic-bezier(0.16, 1, 0.3, 1);
  &:hover { transform: translateY(-6px); box-shadow: 0 16px 48px rgba(0, 0, 0, 0.08); border-color: rgba(166, 27, 46, 0.12);
  ...
  .card-body { padding: 18px;
    .card-title { font-family: var(--font-serif); font-size: 17px; font-weight: 600; color: var(--text-main); letter-spacing: 1px; margin-bottom: 8px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .card-summary { font-size: 13px; color: var(--text-light); line-height: 1.7; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 2; overflow: hidden; text-overflow: ellipsis; min-height: 44px; margin-bottom: 12px; }
...
@media (max-width: 1200px) { .spot-grid { grid-template-columns: repeat(3, 1fr); } }
@media (max-width: 900px) {
  .spot-grid { grid-template-columns: repeat(2, 1fr); gap: 16px; }
  .hero-section { height: 280px; }
  .filter-section .filter-inner, .spot-section .spot-inner { padding: 0 20px; }
  .filter-section .search-box { max-width: 100%; }
}
@media (max-width: 600px) {
  .spot-grid { grid-template-columns: 1fr; }
  .hero-section { height: 240px; }
  .filter-section .filter-inner, .spot-section .spot-inner { padding: 0 16px; }
}
</style>
