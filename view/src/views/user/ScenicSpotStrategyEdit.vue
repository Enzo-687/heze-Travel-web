    ...
    <div class="strategy-save-page">
        <div class="save-container">
            <div class="page-header">
                <span class="section-badge">EDIT</span>
                <h2 class="header-title">修改攻略</h2>
                <p class="header-sub">编辑你的攻略内容，让旅行故事更加完善</p>
            </div>
...
import {
    updateScenciSpotStrategyApi,
    queryScenciSpotStrategyDetailApi
...
const updateScenicSpotStraegy = async () => {
    ...
    try {
        await updateScenciSpotStrategyApi(scenicSpotStraegy as any)
        ElMessage.success('景点攻略修改成功')
        router.push('/scenic-spot-strategy')
    } catch (error: any) {
        console.log('景点攻略修改异常：', error)
        ElMessage.error(`景点攻略修改异常： ${error}`)
    }
}

const getScenicSpotStrategyDetail = async (id: number | null) => {
    if (!id) {
        console.error("景点攻略ID为空")
        return
    }
    try {
        const { data } = await queryScenciSpotStrategyDetailApi(id)
        Object.assign(scenicSpotStraegy, data)
        cover.value = data.cover
        contentWithImage.value = data.content
        scenicSpot.value.scenicSpotId = data.scenicSpotId
        scenicSpot.value.scenicSpotTitle = data.scenicSpotTitle
    } catch (error: any) {
        console.error("查询景点攻略详情异常：", error)
    }
}
